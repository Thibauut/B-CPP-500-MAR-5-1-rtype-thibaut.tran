...
void Parser::parseAction(std::string data) {
    ...
    if (data.find("CREATE_ROOM") != std::string::npos) {
        ...
        int posTitleGame = findQuote(data, 11);
        _args.push_back(data.substr(posTitleGame + 1, findQuote(data, 12) - findQuote(data, 11) - 1));
    }
...
void Parser::callback() {
    switch (_action) {
        ...
        case GET_PLAYER_INFO:
            ...
            break;
        case CREATE_ROOM:
            createRoom(_args.at(0), std::stoi(_args.at(1)), _args.at(2), _args.at(4), std::stoi(_args.at(3)), _args.at(5));
            break;
        case JOIN_ROOM:
...
void Parser::createRoom(std::string player_uuid, int nb_slots, std::string name, std::string pathMap, int gameType, std::string titleGame) {
    ...
    for (std::shared_ptr<PlayerLobby> player : _server->players_) {
        if (player->getUuid() == player_uuid) {
            _server->_lobbys.push_back(std::make_shared<RoomLobby>(player, nb_slots, name, room_uuid, pathMap, gameType, titleGame));
